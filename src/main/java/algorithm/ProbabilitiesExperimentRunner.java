...
public class ProbabilitiesExperimentRunner extends AbstractAlgorithmRunner {
    ...
    private static final int numRuns = 50; // Número de ejecuciones por configuración
...
//Jefes
        ...
        double[] crossoverProbabilities = {0.5, 0.6, 0.7, 0.8, 0.9, 1.0};
        double[] mutationProbabilities = {0.2, 0.1, 0.05, 0.01};
        ...
        int[] startingLengthValues = new int[numRuns];
        for (int run = 0; run < numRuns; run++) {
            tiempoTotalValues[run] = random.nextInt(481) + 120; // Random between 120 and 600
            startingLengthValues[run] = random.nextInt(49) + 12; // Random between 12 and 60
            System.out.println("Tiempo:" + tiempoTotalValues[run] + ", Largo:" + startingLengthValues[run]);
        ...
        try (FileWriter writer = new FileWriter("experiment_results.csv")) {
            ...
            for (double crossoverProbability : crossoverProbabilities) {
                for (double mutationProbability : mutationProbabilities) {
                    ...
                    for (int run = 0; run < numRuns; run++) {
                        int tiempoTotal = tiempoTotalValues[run];
                        int startingLength = startingLengthValues[run];
